...
export default function AdjudantHomePage() {
	...
	useEffect(() => {
		// Fetch only approved soldiers from database
		fetch("/api/soldiers?pending=false")
			.then(res => res.json())
			.then(data => {
				if (Array.isArray(data)) {
					const fitCount = data.filter((s: { fitnessStatus: string }) => s.fitnessStatus === 'Fit').length;
					// Count soldiers who are explicitly 'Unfit' or don't have 'Fit' status
					const unfitCount = data.filter((s: { fitnessStatus: string }) => s.fitnessStatus !== 'Fit').length;
