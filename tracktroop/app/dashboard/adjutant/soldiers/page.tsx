...
export default function SoldiersPage() {
    ...
    const [filterMedicalCategory, setFilterMedicalCategory] = useState<string>("all");
    const [filterFitnessStatus, setFilterFitnessStatus] = useState<string>("all");

    const ranks = useMemo(() => Array.from(new Set(mockSoldiers.map(s => s.rank))), []);

    const filteredSoldiers = useMemo(() => {
        return mockSoldiers.filter((soldier) => {
            ...
            const matchesMedical = filterMedicalCategory === "all" || soldier.medicalCategory === filterMedicalCategory;
            const matchesFitness = filterFitnessStatus === "all" || soldier.fitnessStatus === filterFitnessStatus;
            return matchesServiceNo && matchesName && matchesRank && matchesMedical && matchesFitness;
        });
    }, [filterServiceNo, filterName, filterRank, filterMedicalCategory, filterFitnessStatus]);

    const resetFilters = () => {
        setFilterServiceNo("");
        setFilterName("");
        setFilterRank("all");
        setFilterMedicalCategory("all");
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="medical">Medical Cat.</Label>
                                <Select value={filterMedicalCategory} onValueChange={setFilterMedicalCategory}>
                                    <SelectTrigger id="medical">
